...
| **F008** | Architecture | `app/ai/blueprints/nodes/evaluator_node.py:27`; `app/ai/blueprints/engine.py:580-600` | High | S | `EvaluatorNode` and engine evaluator-revision routing are coded but no `Blueprint` definition under `definitions/` instantiates the node. Phase 48.4's evaluator agent is unreachable. | Either wire it into `definitions/campaign.py` behind `settings.ai.evaluator.enabled`, or delete the node + engine branch + `evaluator_revision_count`. |
| **F009** | Architecture | `app/ai/pipeline/executor.py:63`; `settings.pipeline.enabled=False` (`app/core/config.py:817`) | High | M | ~1,132 LOC of DAG executor + contracts + registry + adapters (Phase 48.1–48.3, 48.5) is never invoked. No production code branches on `settings.pipeline.enabled`. `app/ai/hooks/builtin/adversarial_gate.py` only fires if pipeline is enabled. | Land the engine→executor switch in `app/ai/blueprints/service.py:164` behind the flag, or move `app/ai/pipeline/` to a `prototypes/` branch until it ships. |

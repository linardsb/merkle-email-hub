...
| **F012** | God class | `app/design_sync/service.py:131-1729` (`DesignSyncService`, 49 methods) | Critical | M | **RESOLVED** (Plan 08, Part C). Carved into 6 sub-services under `app/design_sync/services/` — `ConnectionService`, `AssetsService`, `TokenConversionService`, `ImportRequestService`, `WebhookService`, `AccessService` — sharing a `DesignSyncContext` (db + repo + provider cache). `DesignSyncService` remains as a delegating facade so existing route/MCP/blueprint/test patch sites are unchanged. Facade deletion + caller migration tracked in `.agents/plans/tech-debt-08b-design-sync-service-deletion.md`. |

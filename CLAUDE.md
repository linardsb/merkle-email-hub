...
**Export QA Gate:** `app/connectors/qa_gate.py` — `ExportQAGate` evaluates 14 QA checks against configurable blocking/warning classification before ESP export. `ExportQAConfig` per-project JSON column on `Project` overrides global defaults (`EXPORT__QA_GATE_MODE`: enforce/warn/skip). `POST /export/pre-check` dry-run endpoint. Admin-only `skip_qa_gate` override with audit log.

**Export Approval Gate:** `app/connectors/approval_gate.py` — `ExportApprovalGate` checks approval status before ESP export. `Project.require_approval_for_export` boolean column enables per-project. Evaluates latest `ApprovalRequest` for build: approved → pass, pending/rejected/revision_requested → block. Third gate in `ConnectorService.export()` after QA + rendering. Admin-only `skip_approval` override with audit log. Pre-check endpoint includes `approval` result.
...
- **Phase 28** — Export Quality Gates & Approval Workflow (2/3 done — 28.1 QA enforcement in export flow DONE, 28.2 approval workflow → export integration DONE; remaining: 28.3 approval frontend UI)

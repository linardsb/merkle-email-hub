...
- **P48 — Pipeline DAG + adversarial quality loops.** `app/ai/pipeline/` (DAG, executor, artifacts, contracts, hooks); `EvaluatorAgentService` adversarial cross-provider agent; Tree-mode scaffolder + `TreeCompiler`; MCP cache + schema compression; proactive QA warning injection from semantic memory.

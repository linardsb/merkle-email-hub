...
**Per-Agent Sanitization:** `sanitize_html_xss(html, profile=)` applies agent-specific nh3 allowlists. 9 profiles in `app/ai/shared.py`. `BaseAgentService.sanitization_profile` class attribute wires into `_post_process()`.

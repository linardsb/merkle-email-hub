...
>
> **Reference fixtures:** `email-templates/training_HTML/for_converter_engine/` — 3 hand-built campaign HTMLs (Starbucks, Mammut, MAAP) with linked Figma node IDs and a detailed `CONVERTER-REFERENCE.md` mapping every section to its source component, slot fills, and style overrides. These are the converter's ground truth.
...
> **CRITICAL CONTEXT — Hand-Built Reference HTMLs Available.**
> Three production-quality campaign HTMLs already exist in `email-templates/training_HTML/for_converter_engine/` — each built by hand from the component library to match a specific Figma design. These are the **primary reference fixtures** for this subtask:
>
...
>
> All three live in Figma file `VUlWjZGAEVZr3mK1EawsYR`. The companion `CONVERTER-REFERENCE.md` documents every section's source component, slot fills, structural changes, style overrides, and design reasoning. **This is the ground truth the converter must reproduce.**
>
> **Execute this phase diligently.** These reference HTMLs represent the exact transformation patterns (color override, font swap, structural adaptation, VML upgrade, mobile reflow) that the converter must learn to perform automatically. Do not cut corners — every section of every reference HTML must be validated against the converter's output. Differences between converter output and these hand-built HTMLs are **real bugs**, not acceptable approximations.

**What:** Collect 3–5 diverse real Figma email designs as snapshot test cases. Each case needs: diagnostic extract (`structure.json` + `tokens.json`), exported images (`data/design-assets/{id}/`), and a hand-corrected `expected.html` verified against the Figma design. Activate all cases in manifest. Use the 3 hand-built reference HTMLs in `email-templates/training_HTML/for_converter_engine/` as the starting `expected.html` for their corresponding Figma nodes — these are already verified against the designs and should NOT be regenerated from scratch.
...
**Implementation:**
- **Start with the 3 existing reference HTMLs** — copy from `email-templates/training_HTML/for_converter_engine/` into `data/debug/{id}/expected.html` for cases 5 (MAAP), 6 (Starbucks), 10 (Mammut). These are already hand-verified against the Figma designs and document exact component mappings in `CONVERTER-REFERENCE.md`.
- Extract diagnostic data via `python -m app.design_sync.diagnose.extract --connection-id <N> --node-id <node>`:
  - Case 5: MAAP x KASK (done — 13 sections, Figma node `2833-1623`)
  - Case 6: Starbucks promotional (Figma node `2833-1424` — 9 sections: hero, heading, body+CTA, 2-col products, 4-col nav, divider, social, footer, legal)
  - Case 10: Mammut Duvet Day (Figma node `2833-1135` — 18 sections: hero, multi-hero stack, editorial blocks, product grid, CTA sequences, footer)
...
### ~~40.3 Visual Regression: Figma Design Screenshot Capture `[Backend]`~~ DONE
...
**Reference Figma nodes:** All 3 hand-built reference HTMLs in `email-templates/training_HTML/for_converter_engine/` have known Figma node IDs (Starbucks `2833-1424`, Mammut `2833-1135`, MAAP `2833-1623` in file `VUlWjZGAEVZr3mK1EawsYR`). Capture `design.png` for all 3 — these are the highest-value visual regression cases because we have pixel-perfect expected HTML to compare against.
**Implementation:**

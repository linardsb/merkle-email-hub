...
export function useQAResult(resultId: number | null) {
  return useSWR<QAResultResponse>(resultId ? `/api/v1/qa/results/${resultId}` : null, fetcher);
}
...
export function useQALatest(templateVersionId: number | null) {
  return useSWR<QAResultResponse>(
    templateVersionId ? `/api/v1/qa/results/latest?template_version_id=${templateVersionId}` : null,
    fetcher,
  );
}
...
export function useQAResults(
  ...
  return useSWR<PaginatedQAResults>(`/api/v1/qa/results?${params.toString()}`, fetcher);
}

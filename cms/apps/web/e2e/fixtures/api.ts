...
export class ApiHelper {
  ...
  async createProject(name: string) {
    const orgsRes = await fetch(`${BACKEND_URL}/api/v1/orgs?page=1&page_size=1`, {
    ...
    if (!clientOrgId) {
      throw new Error("No client org available; global-setup should have created one");
    }
  ...
  async decideApproval(approvalId: number, decision: string) {
    const res = await fetch(`${BACKEND_URL}/api/v1/approvals/${approvalId}/decide`, {
      method: "POST",
      headers: this.headers(),

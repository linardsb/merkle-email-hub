...
test.describe("Approval Workflow", () => {
  ...
  test("approval cards show status badges", async ({
  ...
  test("approval detail navigation", async ({
    authenticatedPage: page,
  }) => {
    await page.goto("/approvals");
    ...
    if (await approvalCard.isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      await expect(
        page
          .getByText(/preview|qa|quality|build/i)
          .first()
      ).toBeVisible({ timeout: 10_000 });
    }
  });

  test("approve button triggers status change", async ({
    authenticatedPage: page,
  }) => {
    await page.goto("/approvals");
